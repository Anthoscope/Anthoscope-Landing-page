...
const TeamMemberCard = ({ photo, name, caption, links }) => (
  <div className="group flex flex-col items-center p-6 md:p-8 bg-white rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-50 max-w-[320px] w-full mx-auto relative overflow-hidden">
    ...
    <div className="w-40 h-40 md:w-48 md:h-48 mb-6 rounded-2xl overflow-hidden shadow-sm bg-gray-100 flex items-center justify-center">
      <img src={photo} alt={name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
    </div>
    <h5 className="font-black text-gray-900 text-xl md:text-2xl mb-1 text-center">{name}</h5>
    <p className="text-[10px] md:text-sm font-extrabold text-cherry-red uppercase tracking-widest mb-6 text-center">{caption}</p>
...
const AboutPage = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-pink-50/30 selection:bg-cherry-red selection:text-white font-sans overflow-x-hidden">
      <AnimatedBackground />
      
      {/* 1. TOP NAVIGATION - Responsive and Hide-on-scroll */}
      <nav className={`fixed top-0 left-0 w-full z-50 px-4 md:px-8 py-3 md:py-5 flex justify-between items-center bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}>
        <Link to="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
          {/* LOGO FIX: Reduced scale on mobile so it doesn't block the Return link */}
          <img src={`${base}images/anthologo_neg.png`} className="h-10 md:h-16 w-auto object-contain scale-[1.2] md:scale-[2.0] origin-left" alt="Anthoscope Logo" />
        </Link>
        <Link to="/" className="text-[10px] md:text-sm font-bold text-gray-500 hover:text-cherry-red transition-colors uppercase tracking-widest whitespace-nowrap">
          ← Return <span className="hidden sm:inline">to Home</span>
        </Link>
      </nav>

      {/* 2. HERO SECTION */}
      <section className="w-full pt-28 md:pt-40 pb-16 md:pb-24 px-6 relative z-10">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 items-center">
          <div className="text-center lg:text-left">
            <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-gray-900 leading-tight mb-6 md:mb-8">
          ...
          <div className="relative group max-w-2xl mx-auto w-full">
            <div className="absolute -inset-1 bg-gradient-to-r from-cherry-red to-pink-400 rounded-[1.5rem] md:rounded-[2rem] blur opacity-10 group-hover:opacity-30 transition duration-1000"></div>
            <div className="relative rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-2xl border-4 border-white bg-white transition-all duration-500">
              <img src={`${base}images/map.png`} alt="Anthoscope Map Interface" className="w-full h-auto object-cover" />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* 3. THE RITA STORY */}
      <section className="w-full bg-white py-20 md:py-32 px-6 relative z-10">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-10 md:gap-16">
          ...
          <div className="w-full md:w-7/12 text-center md:text-left">
            <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-6">The Rita Story</h2>
            <div className="w-16 h-2 bg-cherry-red mb-8 rounded-full mx-auto md:mx-0"></div>
            ...
            <p className="text-lg md:text-xl text-gray-600 leading-relaxed italic border-l-4 border-gray-200 pl-6 text-left">
      ...
      {/* 4. THE TEAM */}
      <section className="w-full py-20 md:py-32 px-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 md:mb-20">
            <h2 className="text-3xl md:text-5xl font-black text-gray-900 mb-6">The Development Team</h2>
      ...
      <footer className="w-full bg-white py-24 px-6 text-center relative z-10 border-t border-gray-100">
        ...
        <Link to="/" className="inline-block px-10 py-4 md:px-12 md:py-5 bg-cherry-red hover:opacity-90 text-white rounded-full font-black text-lg md:text-xl hover:scale-105 transition-all duration-300 shadow-xl hover:shadow-cherry-red/30">
          Launch Anthoscope
        </Link>
      </footer>
    </div>
